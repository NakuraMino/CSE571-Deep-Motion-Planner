...
#### AStarNet
The NN we implemented is a classifier that takes in the current positionl and the goal position and a map, and outputs probabilities for choosing each of the 8 actions for the current step. See [src/astaret.py](./src/astarnet.py) for more details.
...
We implemented a simple replanning algorithm that removes intermediate states in paths that were unnecessary to take during the bidirectional planning phase.

![Lazy state contraction](./figs/LSC.png)

#### Results
left: path generated by A\* (cost = 84.45), 
middle: path generated by AStarNet with bidirectional planner (cost = 84.87), 
right: path generated by AStarNet with unidirectional planner (cost = 86.70)
![Astar](./figs/astar.png)
![bidirectional planner](./figs/bidirectional-planner.png)
![unidirectional planner](./figs/unidirectional-planner.png)
...
#### RRTnet
The NN takes in the start position, orientation, goal position, orientation, and the map as the input and outputs the linear velocity and orientation. A significant difference from the AStarNet is that we include a dropout layer because their randomness can help us recover from failed predictions (i.e., the same input can give different outputs). If the network produces an invalid action, we can pass the same input again to obtain a different motions. See [src/rrtnet.py](./src/rrtnet.py) for more details.
...
#### Results
left: path generated by nonholonomic RRT (cost = 220)
right: path generated by RRTNet with dropout planner (cost = 504, dropout probability = 0.7)
![Path generated by RRT supervisor](./figs/rrt.png)
![Path generated by RRTNet](./figs/rrtnet.png)
