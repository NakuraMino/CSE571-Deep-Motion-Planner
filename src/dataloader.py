...
class NonHolonomicDataset(Dataset):
    """
    The dataset object for the Holonomic robot data.
    To be used with `torch.utils.data.DataLoader`

    For reference, columns in data.csv are:
    curr_y, curr_x, curr_theta, goal_y, goal_x, goal_theta mapImage_path, velocity, angle
    """
    ...
    def __getitem__(self, i):
        ...
        returns: a tuple (ipt, opt) where,
        - ipt is a tuple containing 2 elements: (positions, map_image)
            * positions: a 1d torch tensor of length 6
              containing [curr_y, curr_x, curr_theta, goal_y, goal_x, goal_theta]
            * map_image: a 2d torch tensor of size 128 x 128
              containing map img (if grayscale, otherwise it is 128 x 128 x 3)
        - opt is a 1d torch tensor of size 2
          containing the action to be performed (linear velocity and angle)
        """
        N = len(self.data)
        ...
        if i < N:
            # Input
            positions = torch.tensor(self.data.iloc[i][:6])
            img_path = os.path.join(self.path, self.data.iloc[i][6][2:])
            if self.grayscale is True:
                map_image = torch.from_numpy(cv2.imread(img_path, 0))
            else:
                map_image = torch.from_numpy(cv2.imread(img_path, 1))
            ...
            # Output
            opt = torch.tensor(self.data.iloc[i][7:].values.astype(np.float64))
            return (ipt, opt)
...
if __name__ == '__main__':
    csv_file = 'data.csv'
    path = '../nonholonomic_data/'
    grayscale = True
    full_dataset = NonHolonomicDataset(csv_file, path, grayscale=grayscale)
